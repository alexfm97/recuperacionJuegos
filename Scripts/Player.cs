...
public class Player : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag == "EnemyGreen") // If the player collides with an enemy
        {
        ...
        }else if(collision.gameObject.tag == "EnemyYellow")
        {
            isAlive = false; // The player is not alive
            transform.GetChild(0).gameObject.SetActive(false); // Deactivate the fire
            GetComponent<CircleCollider2D>().enabled = false; // Deactivate the collider
        ...
        private void OnCollisionEnter2D(Collision2D collision)
        {
            if (collision.gameObject.tag == "EnemyGreen" || collision.gameObject.tag == "EnemyYellow" || collision.gameObject.tag == "EnemyPink" || collision.gameObject.tag == "LaserEnemy") // If the player collides with an enemy
            {
                /*isAlive = false; // The player is not alive
                transform.GetChild(0).gameObject.SetActive(false); // Deactivate the fire
                GetComponent<CircleCollider2D>().enabled = false; // Deactivate the collider
